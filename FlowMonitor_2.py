...
class FlowMonitor:
	...
	def update_queues(self, controller_message):
		...
		# Here we should:
		# 1. Init and set bw with the new queue list
		...
		for i in range(len(controller_message['update_queue'])):
			for j in range(len(self.complete_interface_list[0]['queueList'])):
				if (controller_message['update_queue'][i]['nw_src'] == self.complete_interface_list[0]['queueList'][j]['nw_src']) and (controller_message['update_queue'][i]['nw_dst'] == self.complete_interface_list[0]['queueList'][j]['nw_dst']):
					self.complete_interface_list[0]['queueList'][j]['bw'] = controller_message['update_queue'][i]['bw']				

		# Append new queues to the global queue list
		for i in range(len(new_queue_list)):
			self.complete_interface_list[0]['queueList'].append(new_queue_list[i])

		delete_indexes = []

		for i in range(len(controller_message['delete_queue'])):
			for j in range(len(self.complete_interface_list[0]['queueList'])):
				if (controller_message['delete_queue'][i]['nw_src'] == self.complete_interface_list[0]['queueList'][j]['nw_src']) and (controller_message['delete_queue'][i]['nw_dst'] == self.complete_interface_list[0]['queueList'][j]['nw_dst']):
